...
help:
    ...
    @echo "  codeql-analyze           - Analyze 'codeql-db' with the Python query pack and write results.sarif"
...
codeql-create-db:
    ...
    "$CMD" database create codeql-db --language=python --command "$BUILD_CMD" $OVERWRITE_FLAG

codeql-analyze:
    @echo "Analyzing 'codeql-db' (use PACK and OUT env vars to override)"
    PACK="${PACK:-codeql/python-queries@latest}"; \
